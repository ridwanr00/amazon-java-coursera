...
public class Main {
     ...
     */
    static void writeObjectsToFile(Tiger tiger, Penguin penguin, Dolphin dolphin) {
        try (ObjectOutputStream tigerObject = new ObjectOutputStream(new FileOutputStream("tiger.txt"))) {
            tigerObject.writeObject(tiger);
            tigerObject.close();
            System.out.println("Tiger object has been serialized to tiger.txt");
        } catch (IOException e) {
            e.printStackTrace();
        }

        try (ObjectOutputStream penguinObject = new ObjectOutputStream(new FileOutputStream("penguin.txt"))) {
            penguinObject.writeObject(penguin);
            penguinObject.close();
            System.out.println("Penguin object has been serialized to penguin.txt");
        } catch (IOException e) {
            e.printStackTrace();
        }

        try (ObjectOutputStream dolphinObject = new ObjectOutputStream(new FileOutputStream("dolphin.txt"))) {
            dolphinObject.writeObject(dolphin);
            dolphinObject.close();
            System.out.println("Dolphin object serialized to dolphin.txt");
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    static void readObjectsFromFile() {
        try (ObjectInputStream tigerObject = new ObjectInputStream(new FileInputStream("tiger.txt"))) {
            Tiger tiger = (Tiger) tigerObject.readObject(); // Cast required
            tigerObject.close();
            System.out.println("Deserialized Tiger object:");
            System.out.println(tiger);
        } catch (IOException | ClassNotFoundException e) {
            e.printStackTrace();
        }

        try (ObjectInputStream penguinObject = new ObjectInputStream(new FileInputStream("penguin.txt"))) {
            Penguin penguin = (Penguin) penguinObject.readObject(); // Cast required
            penguinObject.close();
            System.out.println("Deserialized Penguin object:");
            System.out.println(penguin);
        } catch (IOException | ClassNotFoundException e) {
            e.printStackTrace();
        }

        try (ObjectInputStream dolphinObject = new ObjectInputStream(new FileInputStream("dolphin.txt"))) {
            Dolphin dolphin = (Dolphin) dolphinObject.readObject(); // Cast required
            dolphinObject.close();
            System.out.println("Deserialized Dolphin object:");
